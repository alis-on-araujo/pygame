...
diamante_largura = 40
diamante_altura =40
diamante_img = pygame.image.load('imagens/diamante.png').convert_alpha()
diamante_img = pygame.transform.scale(diamante_img, (diamante_largura, diamante_altura))
...
while game:
    ...
    if timer_started:
        if current_time >= 1 and timer == 0:
            ...
            for i in range(1):
                diamante = Diamante(diamante_img)
                todospassaros.add(diamante)
                todosdiamantes.add(diamante)
    ...
    hits_3 = pygame.sprite.spritecollide(jogador_balões, todosdiamantes, True)
    for hit in hits_2:
        for i in range(1):
            diamante = Diamante(diamante_img)
            todospassaros.add(diamante)
            todasestrelas.add(diamante)
