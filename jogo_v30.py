...
assets['tela_inicio'] = pygame.image.load('imagens/inicial.png').convert()
assets['tela_inicio'] = pygame.transform.scale(assets['tela_inicio'], (700, 850))
assets['tutorial'] = pygame.image.load('imagens/Instruções.png').convert()
assets['tutorial'] = pygame.transform.scale(assets['tutorial'], (700, 850))
assets['gameover'] = pygame.image.load('imagens/GAME OVER.png').convert()
assets['gameover'] = pygame.transform.scale(assets['gameover'], (700, 850))
...
tempo_anterior_passaro = tempo_atual
...
while game:
    ...
    # criando passaros
    if tempo_atual - tempo_anterior_passaro >= 10000:
        for i in range(1):
            passaro = Passaro(assets)
            todos_sprites.add(passaro)
            todos_passaros.add(passaro)
    ...
    if pode_cair_estrela and tempo_atual - tempo_anterior_estrela >= 1000 and contador_estrelas < 5:
        tempo_anterior_estrela = tempo_atual
    ...
    if pode_cair_vida and tempo_atual - tempo_anterior_vida >= 25000 and contador > 1:
        tempo_anterior_vida = tempo_atual
   ...
   # Passaros batem no balão se a casa não estiver imune
    ...
    text_surface = font.render('{:1d}'.format(score_estrela), True, YELLOW)
    text_rect = text_surface.get_rect()
    text_rect.midtop = (largura-618, 30)
    window.blit(text_surface, text_rect)
    todos_sprites.draw(window)
